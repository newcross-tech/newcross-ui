...
import { BadgeContentProps, BadgePositions, BadgeSizes } from './LegacyBadge.types';
...
const getCutoutStyles = ({ size, position }: Partial<LegacyBadgeProps>) => {
  if (position && size) {
...
export const Cutout = styled.div`
  ${({ size, position, hasCutout }: Partial<LegacyBadgeProps>) => hasCutout && getCutoutStyles({ size, position })}
`;
